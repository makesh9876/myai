...
#             )
#         Message.objects.create(user=user, role="human", text=user_message)
#         past_messages = Message.objects.filter(user=user).order_by("-timestamp")[
#             :10
#         ]  # Get latest 10
#         past_messages = list(reversed(past_messages))
#         conversation_history = [(msg.role, msg.text) for msg in past_messages]
...
#     "GOOGLE_API_KEY", "AIzaSyAl9JZd6Kr98NcTD-KSap4jz0HDemrHbcg"
# )
...
#             Message.objects.create(user=user, role="ai", text=ai_response.content)
#             return Response(
#                 {"response": ai_response.content}, status=status.HTTP_200_OK
#             )
#         except Exception as e:
#             return Response(
